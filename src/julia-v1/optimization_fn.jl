...
function calc_optimization_val(darp::DARP, routes::Dict{int, Array{int}}, darp)
    route_values = Dict(k => route_values(route, darp) for k in keys(routes))
    ...
    function late_quantity(k, i)
        ...
        Bi_pickup = route_values[k][3][i]
        Bi_dropoff = route_values[k][3][-i]
